...
use imacs::spec::{ConditionValue, Output, Rule, Spec, VarType, Variable, WhenClause};
use rstest::rstest;
...
fn make_2_bool_spec(complete: bool) -> Spec {
    let mut rules = vec![
        Rule {
            id: "R1".into(),
            when: Some(WhenClause::Single("a && b".into())),
            conditions: None,
    ...
    Spec {
        ...
        rules: (0..8)
            .map(|i| {
                ...
                Rule {
                    id: format!("R{}", i + 1),
                    when: Some(WhenClause::Single(conditions.join(" && "))),
                    conditions: None,
...
fn make_overlapping_spec() -> Spec {
    Spec {
        ...
        rules: vec![
            Rule {
                id: "R1".into(),
                when: Some(WhenClause::Single("a".into())),
                conditions: None,
                then: Output::Single(ConditionValue::Int(1)),
                priority: 0,
                description: None,
            ...
            Rule {
                id: "R2".into(),
                when: Some(WhenClause::Single("a".into())), // Same condition!
                conditions: None,
                then: Output::Single(ConditionValue::Int(2)),
                priority: 0,
                description: None,
...
fn make_comparison_spec() -> Spec {
    Spec {
        ...
        rules: vec![
            Rule {
                id: "R1".into(),
                when: Some(WhenClause::Single("amount > 1000".into())),
                conditions: None,
                then: Output::Single(ConditionValue::Int(1)),
                priority: 0,
                description: None,
...
fn make_equality_spec() -> Spec {
    Spec {
        ...
        rules: vec![
            Rule {
                id: "R1".into(),
                when: Some(WhenClause::Single(r#"status == "active""#.into())),
                conditions: None,
                then: Output::Single(ConditionValue::Int(1)),
                priority: 0,
                description: None,
            ...
            Rule {
                id: "R2".into(),
                when: Some(WhenClause::Single(r#"status == "inactive""#.into())),
                conditions: None,
                then: Output::Single(ConditionValue::Int(0)),
                priority: 0,
                description: None,
...
fn make_membership_spec() -> Spec {
    Spec {
        ...
        rules: vec![
            Rule {
                id: "R1".into(),
                when: Some(WhenClause::Single(r#"region in ["US", "EU"]"#.into())),
                conditions: None,
                then: Output::Single(ConditionValue::Int(1)),
                priority: 0,
                description: None,
            ...
            Rule {
                id: "R2".into(),
                when: Some(WhenClause::Single(r#"region == "APAC""#.into())),
                conditions: None,
                then: Output::Single(ConditionValue::Int(0)),
                priority: 0,
                description: None,
...
fn make_spec_with_rule(when: &str) -> Spec {
    Spec {
        ...
        rules: vec![Rule {
            id: "R1".into(),
            when: Some(WhenClause::Single(when.into())),
            conditions: None,
            then: Output::Single(ConditionValue::Int(1)),
            priority: 0,
            description: None,
