    ...
    if !issue.affected_rules.is_empty() {
        ...
        Some(SpecFix {
            ...
            operation: FixOperation::UpdateRule {
                ...
                old_value: rule.when.as_ref()?.clone(),
    ...
    for rule in &spec.rules {
        if let Some(when) = &rule.when {
            if let Err(e) = check_cel_types(when, &var_types) {
                ...
                let vars_in_expr: Vec<String> = extract_variable_names(when)
                    .into_iter()
...
/// Extract variable names from a CEL expression
fn extract_variable_names(expr: &str) -> Vec<String> {
    // Simple extraction: find identifiers that look like variable names
    let re = regex::Regex::new(r"\b([a-zA-Z_][a-zA-Z0-9_]*)\b").unwrap();
    let keywords = ["true", "false", "null", "in", "and", "or", "not"];
...
fn detect_contradictions(spec: &Spec, code_counter: &mut usize) -> Vec<ValidationIssue> {
    ...
    for (i, rule_a) in spec.rules.iter().enumerate() {
        for rule_b in spec.rules.iter().skip(i + 1) {
            if rule_a.when.is_some() && rule_b.when.is_some() {
