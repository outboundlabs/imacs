...
pub struct OutputConfig {
    /// Default output directory (relative to imacs folder parent)
    /// If not specified, falls back to "./generated"
    pub default: Option<String>,

    /// Per-language output directories (relative to imacs folder parent)
...
pub struct ValidationConfig {
    ...
    #[serde(default = "default_true")]
    pub detect_output_conflicts: bool,
}
...
pub struct LocalConfig {
    ...
    /// Override output directory configuration
    pub output: Option<OutputConfig>,
}
...
impl ImacRoot {
    ...
    pub fn merge(&self, local: Option<&LocalConfig>) -> MergedConfig {
        ...
        // Merge output config: local overrides root, root defaults to empty
        let root_output = self.defaults.output.clone().unwrap_or_default();
        let merged_output = if let Some(local_output) = &local.output {
            // Merge: local values override root, but root defaults fill in gaps
            OutputConfig {
                default: local_output.default.clone().or(root_output.default.clone()),
                rust: local_output.rust.clone().or(root_output.rust.clone()),
                typescript: local_output
                    .typescript
                    .clone()
                    .or(root_output.typescript.clone()),
                python: local_output.python.clone().or(root_output.python.clone()),
                go: local_output.go.clone().or(root_output.go.clone()),
                java: local_output.java.clone().or(root_output.java.clone()),
                csharp: local_output.csharp.clone().or(root_output.csharp.clone()),
            }
        } else {
            root_output
        };
        ...
        MergedConfig {
            ...
            auto_format: local.auto_format.unwrap_or(self.defaults.auto_format),
            naming: local
                .naming
                .clone()
...
mod tests {
    ...
    fn test_merge_output_config() {
        ...
        // Local typescript override should be present
        assert_eq!(merged.output.typescript, Some("./ts_gen".to_string()));
        // Root rust override should be present (not overridden)
        assert_eq!(merged.output.rust, Some("./rust_gen".to_string()));
        // Root default should be present (not overridden)
        assert_eq!(merged.output.default, Some("./generated".to_string()));
    }
}
