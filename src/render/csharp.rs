...
impl<'a> CSharpRenderer<'a> {
    fn render(&self, spec: &Spec) -> csharp::Tokens {
        ...
        // Check if we have a C# namespace configured
        let namespace = self.config.namespace.as_ref().and_then(|ns| {
            if let ResolvedNamespace::CSharp(csharp_ns) = ns {
                Some(csharp_ns.render())
            } else {
                None
            }
        });

        let inner_classes = self.render_classes(spec, &class_name, return_type);
