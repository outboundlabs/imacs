...
/// Render spec to target language
pub fn render(spec: &Spec, target: Target) -> String {
    Renderer::new(target).render(spec)
}
...
impl Renderer {
    ...
    pub fn render(&self, spec: &Spec) -> String {
        match self.target {
            Target::Rust => rust::render(spec, &self.config),
            Target::TypeScript => typescript::render(spec, &self.config),
            Target::Python => python::render(spec, &self.config),
            Target::CSharp => csharp::render(spec, &self.config),
            Target::Java => java::render(spec, &self.config),
            Target::Go => go::render(spec, &self.config),
        }
    }
}
