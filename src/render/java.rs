...
impl<'a> JavaRenderer<'a> {
    fn render(&self, spec: &Spec) -> java::Tokens {
        ...
        // Check if we have a Java package configured
        let package = self.config.namespace.as_ref().and_then(|ns| {
            if let ResolvedNamespace::Java(java_pkg) = ns {
                Some(java_pkg.render())
            } else {
                None
            }
        });
