...
impl<'a> GoRenderer<'a> {
    fn render(&self, spec: &Spec) -> go::Tokens {
        ...
        // Get package name from config or fall back to spec ID (lowercased per Go conventions)
        let package_name = self
            .config
            .namespace
            .as_ref()
            .and_then(|ns| {
                if let ResolvedNamespace::Go(go_pkg) = ns {
                    Some(go_pkg.render())
                } else {
                    None
                }
            })
            .unwrap_or_else(|| spec.id.to_lowercase());

        // Get optional module path comment
